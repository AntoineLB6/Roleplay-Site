...
module.exports.getDiscordInfos = async (req, res) => {
    if (req.body.userID.length !== 18) return res.status(500).send("Invalid ID")
    const thisUser = await server.discordClient.users.fetch(req.body.userID)
    const userData = {
        tag: thisUser.tag,
        avatarURL: thisUser.avatarURL()
    }
    if (thisUser) return res.status(201).json(userData);
    else return res.status(200).send("Compte discord introuvable");
}